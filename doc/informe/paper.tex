...
is calculated in this step. The magnitude of this force will depend
on two factors: by each of the other pedestrians and the walls that
might exists surronding $i$. The final value of the force applied
over the future $i'$ will be the addition of the latest two factors.


The fist factor is caluclated for each of the pedestrian $j$ ($j\neq i$).
First, we filter all pedestrians who are not in the range of sight
of pedestrian $i$. This condition is verified using the following
formula:


\[
\overrightarrow{ii'}\bullet\overrightarrow{ij'}<0
\]
...
doing. Hence, they are ignored. Once filtered, the pedestrian will
try to face towards the direction that think is best for avoiding
all the obstacles. This factor is represented using the social force
formula from Helbing's model.
...
The seocond term, represents the same fact but taking in mind the
current location of the pedestrian $j$. The same law applies to each
of the walls on the simulation, the difference is that the closest
point on the wall and and the future has to be calculated before applying
the formula. Also, the constants $\alpha_{fw}$ and $\beta_{fw}$
have different values.


\begin{figure}[H]
\centering{}\includegraphics[scale=0.5]{pedestrian-top-forces}\protect\caption{External forces affecting future $i'$}
\end{figure}



Then, the desire force that pedestrian $i$ will aply to his future
is calculated. This force is modelled using two springs. The first
spring hast its origin attached on $i$ and its end attached on $i'$
and has a spring constant $K_{1}$. This spring will regulate the
velocity that will be calculated on a previous step at witch the pedestrian
$i$ will advance. The second spring, has its origin on $i'$ and
the end attached to the closest point on the pedestrian target area
and has a spring constant $K_{2}$. In order to avoid an oscilatory
movement (in example, with a single pedestrian), a damping $\gamma$
was added to this second spring. This spring represents how much te
pedestrian wants to reach his target. A larger spring constant will
force a more straight path, but possible with more collitions on its
way.
