...
\\
\iffalse
This forces will try to keep the FVP aligned with $\mathbf{o}$ and at the same time, 
keep the FVP at $RD$ from the pedestrian. 

This (first) spring will regulate the velocity
that will be calculated on a posterior step at which the pedestrian
$i$ will advance. 
...
which pushes the FVP towards its desired length.
At the same time, a force pushing the FVP towards his original
alingment, defined
...
\begin{equation}
\iffalse
F_s(r,\theta)=K_s(\theta)\frac{\vec{ii'}}{\mid\vec{ii'}\mid}(r_{max}-r_{ii'})
\fi
F_{internal}(i)=K_{1}*(|r|-RD)+K_{2}(\theta_{i})+K_{2}(\theta_{i})*(???)
\end{equation}

\begin{figure}[H]
\centering{}\includegraphics[scale=0.5]{pics/pedestrian-internal-forces}\protect\caption{Pedestrian internal forces}
\end{figure}
...
\\
\\
This force will produce avoidance movements when obstactes are detected on the path.
...
does not make decisions based on obstacles behind him. Hence,
they are ignored. The formula for the external forces that the
FVP $i$ feels is defined as follows
...
\iffalse
The first term of the sum represents
